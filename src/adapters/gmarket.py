...
class GmarketAdapter(BaseAdapter):
    ...
    async def search_by_image(self, context, image_path: str, query_hint: str | None) -> list[ListingCandidate]:
        ...
        url = f"https://browse.gmarket.co.kr/search?keyword={quote(query_hint)}"
        ...
        link_re = re.compile(r'<a[^>]+class="link__item"[^>]+href="([^"]+)"[^>]*>(.*?)</a>', re.S)
        rank = 0
        for m in link_re.finditer(html):
            href = m.group(1)
            title = _strip_html(m.group(2))
            if not href.startswith("http") or len(title) < 4:
                continue
            rank += 1
            if rank > self.max_candidates:
                break
            item_id = re.sub(r"\W+", "_", href)[-30:] or f"gm_{rank}"
            candidates.append(
                ListingCandidate(
