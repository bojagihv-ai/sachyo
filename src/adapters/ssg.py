...
class SsgAdapter(BaseAdapter):
    ...
    async def search_by_image(self, context, image_path: str, query_hint: str | None) -> list[ListingCandidate]:
        ...
        url = f"https://www.ssg.com/search.ssg?target=all&query={quote(query_hint)}"
        ...
        for m in link_re.finditer(html):
            ...
            if len(title) < 4:
                continue
            if href.startswith("/"):
                href = f"https://www.ssg.com{href}"
            if not href.startswith("http"):
                continue
            rank += 1
            if rank > self.max_candidates:
                break
            item_id = re.sub(r"\W+", "_", href)[-30:] or f"ssg_{rank}"
