...
class WemakepriceAdapter(BaseAdapter):
    source_name = "wemakeprice"
    ...
    async def search_by_image(self, context, image_path: str, query_hint: str | None) -> list[ListingCandidate]:
        ...
        url = f"https://search.wemakeprice.com/search?search_cate=top&keyword={quote(query_hint)}"
        ...
        for m in re.finditer(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', html, re.S):
            ...
            if len(title) < 4:
                continue
            if href.startswith("/"):
                href = f"https://front.wemakeprice.com{href}"
            if not href.startswith("http"):
                continue
            rank += 1
            if rank > self.max_candidates:
                break
            item_id = re.sub(r"\W+", "_", href)[-30:] or f"wmp_{rank}"
