...
class LotteOnAdapter(BaseAdapter):
    source_name = "lotteon"
    ...
    async def search_by_image(self, context, image_path: str, query_hint: str | None) -> list[ListingCandidate]:
        ...
        url = f"https://www.lotteon.com/search/search/search.ecn?render=search&platform=pc&q={quote(query_hint)}"
        ...
        for m in re.finditer(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', html, re.S):
            ...
            if len(title) < 4:
                continue
            if href.startswith("/"):
                href = f"https://www.lotteon.com{href}"
            if not href.startswith("http"):
                continue
            rank += 1
            if rank > self.max_candidates:
                break
            item_id = re.sub(r"\W+", "_", href)[-30:] or f"lot_{rank}"
