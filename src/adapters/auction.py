...
class AuctionAdapter(BaseAdapter):
    ...
    async def search_by_image(self, context, image_path: str, query_hint: str | None) -> list[ListingCandidate]:
        ...
        url = f"https://browse.auction.co.kr/search?keyword={quote(query_hint)}"
        ...
        for m in link_re.finditer(html):
            ...
            if not href.startswith("http") or "auction.co.kr" not in href or len(title) < 4:
                continue
            rank += 1
            if rank > self.max_candidates:
                break
            item_id = re.sub(r"\W+", "_", href)[-30:] or f"au_{rank}"
            candidates.append(
                ListingCandidate(
